...
- `docs/specs/feature-*.md` — standalone feature specs for work that postdates the product spec (e.g. `feature-static-assessment.md`), each with its own FR numbering and its task list under `docs/tasks/`.

Read `docs/spec.md` before implementing anything; it is the authoritative requirement set. For a capability it does not cover, check `docs/specs/` for a feature spec that does.
